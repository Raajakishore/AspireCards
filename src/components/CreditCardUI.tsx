...
export const CreditCardCarousel = (): React.ReactElement => {
  const cardDetails = useSelector(
    (state: storeType) => state.cards.cardDetails,
  );
    ...
    <View style={styles.container}>
      {cardDetails.length === 0 ? (
        ...
        <Carousel
          ...
          onSnapToItem={(index) => {
            dispatch(
              setSelectedCardIdx({ selectedCardIdx: cardDetails[index].id }),
            );
          }}
        />
      )}
    ...
    <View>
      <View style={styles.cardVisibilityViewStyle}>
        ...
        >
          {cardNumberVisibility ? (
            <HideEyeSVG width={16} height={16} />
          ) : (
            <ShowEyeSVG width={16} height={16} />
          )}
          <Text
            style={{
          ...
          >
            {" "}
            {cardNumberVisibility ? "Hide Card Number" : "Show Card Number"}
          </Text>
        </TouchableOpacity>
      </View>
      ...
      <View
        style={[
          ...
          isCardFreezed && { backgroundColor: colors.background.mediumGreen },
        ]}
      >
        ...
        <View style={styles.nameViewStyle}>
          <Text testID="card-name" style={styles.nameTextStyle}>
            {name}
          </Text>
        </View>

        <View style={styles.cardNumViewStyle}>
          <Text style={styles.cardNumTextStyle}>
            {" "}
            {cardNumberVisibility ? cardNum.slice(0, 4) : "****"}{" "}
          </Text>
          <Text style={styles.cardNumTextStyle}>
            {" "}
            {cardNumberVisibility ? cardNum.slice(4, 8) : "****"}
          </Text>
          <Text style={styles.cardNumTextStyle}>
            {" "}
            {cardNumberVisibility ? cardNum.slice(8, 12) : "****"}
          </Text>
        ...
        <View style={styles.expAndCvvViewStyle}>
          <Text
            testID="expiry-date"
            style={styles.cardInfoTextStyle}
          >{`Thru: ${expiryDate}`}</Text>
          <Text
            testID="cvv"
            style={styles.cardInfoTextStyle}
          >{`CVV: ${cardNumberVisibility ? cvv : "***"}`}</Text>
        </View>
